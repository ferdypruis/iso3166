...
func ExampleFromAlpha3() {
	country, _ := iso3166.FromAlpha3("USA") // Ignoring error for simplicity
	fmt.Println("The two-letter alphabetic code for the", country.Name(), "is", country.Numeric())

	// Output:
	// The two-letter alphabetic code for the US is 840
}

func ExampleFromNumeric() {
	country, _ := iso3166.FromNumeric("666") // Ignoring error for simplicity
	fmt.Println(country.Alpha2(), "is the two-letter code for", country.Name())

	// Output:
	// PM is the two-letter code for St. Pierre & Miquelon
}
